 ...
 * 
 * נתוני מוצרים מ-GA4: נצפים, נמכרים, המרות
 */

import { NextRequest, NextResponse } from 'next/server';
import { getProductsAnalytics, getValidCredentials } from '@/lib/google-analytics';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getProductsAnalytics(credentials, { startDate, endDate });
